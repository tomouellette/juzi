...
def make_adata(
    ...
    """AnnData fit through nmf to similarity, ready for clustering."""
...
def test_error_threshold_above_one():
    with pytest.raises(ValueError):
        jz.gp.programs_cluster(make_adata(), threshold=1.1)


def test_error_threshold_below_zero():
    with pytest.raises(ValueError):
        jz.gp.programs_cluster(make_adata(), threshold=-0.1)


def test_error_min_cluster_below_one():
    with pytest.raises(ValueError):
        jz.gp.programs_cluster(make_adata(), min_cluster=0)
...
def test_output_fields_present():
    adata = make_adata()
    jz.gp.programs_cluster(adata, min_cluster=1)
...
def test_cluster_rerun_resets_keep_cluster():
    ...
    jz.gp.programs_cluster(adata, threshold=0.1, min_cluster=1)
    keep_loose = adata.uns["juzi_keep_cluster"].sum()
    jz.gp.programs_cluster(adata, threshold=0.5, min_cluster=2)
...
def test_low_threshold_fewer_clusters():
    ...
    jz.gp.programs_cluster(adata_low, threshold=0.1, min_cluster=1)
    jz.gp.programs_cluster(adata_high, threshold=0.9, min_cluster=1)
...
def test_threshold_one_each_factor_own_cluster():
    ...
    jz.gp.programs_cluster(adata, threshold=1.0, min_cluster=1)
    labels = adata.uns["juzi_cluster_labels"]
...
def test_min_cluster_removes_small_clusters():
    adata = make_adata(n_samples=6, seed=0)
    min_cluster = 3
    jz.gp.programs_cluster(adata, threshold=0.3, min_cluster=min_cluster)
    labels = adata.uns["juzi_cluster_labels"]
...
def test_reorder_largest_cluster_first():
    adata = make_adata(n_samples=6, seed=0)
    jz.gp.programs_cluster(adata, threshold=0.3, min_cluster=1, reorder=True)
    labels = adata.uns["juzi_cluster_labels"]
...
def test_reorder_false_runs():
    ...
    jz.gp.programs_cluster(adata, threshold=0.3, min_cluster=1, reorder=False)
    assert "juzi_cluster_labels" in adata.uns
...
def test_copy_false_modifies_inplace():
    adata = make_adata()
    result = jz.gp.programs_cluster(adata, min_cluster=1, copy=False)
    assert result is None
    assert "juzi_cluster_labels" in adata.uns


def test_copy_true_returns_new_object():
    adata = make_adata_similarity()
    result = jz.gp.programs_cluster(adata, min_cluster=1, copy=True)
...
def test_select_threshold_with_min_cluster_and_intra_sample_false():
    ...
    jz.gp.similarity_compute(adata, distance="jaccard", top_k=20, intra_sample=False)
...
def test_select_threshold_metric_values_finite_where_valid():
    ...
    adata = make_adata(n_samples=6, seed=0)
    jz.gp.programs_threshold(adata, min_cluster=1, metric="ratio")
...
def test_select_threshold_optimal_is_valid_threshold():
    ...
    optimal = jz.gp.programs_threshold(adata, min_cluster=1, thresholds=thresholds)
    assert any(np.isclose(optimal, t) for t in thresholds)


# juzi.gp.programs_remove
...
class TestProgramsRemove:
    ...
    def test_error_missing_juzi_keep_cluster(self):
        adata = make_adata()
        del adata.uns["juzi_keep_cluster"]
        with pytest.raises(KeyError):
            jz.gp.programs_remove(adata, clusters=[0])
    ...
    def test_error_empty_clusters(self):
        adata = make_adata()
        with pytest.raises(ValueError):
            jz.gp.programs_remove(adata, clusters=[])

    def test_error_invalid_label(self):
        adata = make_adata()
        with pytest.raises(ValueError):
            jz.gp.programs_remove(adata, clusters=[999])

    def test_error_duplicate_labels(self):
        adata = make_adata()
        with pytest.raises(ValueError):
    ...
    def test_juzi_keep_recomputed(self):
        """juzi_keep must equal intersection of all three masks after remove."""
        adata = make_adata()
        jz.gp.programs_remove(adata, clusters=[0])
        expected = (
            adata.uns["juzi_keep_prune"]
    ...
    def test_jackknife_dropped_after_remove(self):
        """juzi_jackknife must be dropped since reference programs changed."""
        adata = make_adata()
    ...
    def test_copy_true_returns_new_object(self):
        ...
        # Result has one fewer program
        assert len(np.unique(result.uns["juzi_cluster_labels"])) == n_before - 1
